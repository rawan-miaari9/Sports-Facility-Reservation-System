          ...
          <div className="bg-white p-6 rounded-2xl border border-outline-variant shadow-sm space-y-4">
            ...
            <div className="divide-y divide-outline-variant/60">
              <div className="flex items-center justify-between py-4">
                <div>
                  <span className="block font-display font-bold text-sm text-on-surface">Email Confirmation Dispatch</span>
                  <span className="block text-[11px] text-outline mt-0.5">Dispatches secure receipts and PDF calendars on slot locks.</span>
                </div>
                <button
                  type="button"
                ...
                >
                  {notifEmail ? (
                    ...
                    <ToggleLeft className="h-8 w-8 text-outline-variant" />
                  )}
                </button>
              </div>
              ...
              <div className="flex items-center justify-between py-4">
                <div>
                  <span className="block font-display font-bold text-sm text-on-surface">SMS Countdown Alert Triggers</span>
                  <span className="block text-[11px] text-outline mt-0.5">Triggers phone SMS texts 30 minutes before your court time locks in.</span>
                </div>
                <button
                  type="button"
              ...
              <div className="flex items-center justify-between py-4">
                <div>
                  <span className="block font-display font-bold text-sm text-on-surface">External Calendar Synclocks</span>
                  <span className="block text-[11px] text-outline mt-0.5">Attempts OAuth sync with Google and Outlook calendars on booking.</span>
                </div>
                <button
                  type="button"
              ...
              <div className="flex items-center justify-between py-4">
                <div>
                  <span className="block font-display font-bold text-sm text-on-surface">Prevent Overlapping Blocks</span>
                  <span className="block text-[11px] text-outline mt-0.5">Double-locks database entries to guarantee zero double bookings.</span>
                </div>
                <button
                  type="button"
                  onClick={() => setDoubleLock(!doubleLock)}
                  className="text-primary transition-all cursor-pointer"
                >
                  {doubleLock ? (
                    <ToggleRight className="h-8 w-8" />
